...
const getAllAuthors = async () => {
	const [authors] = await connection.execute(
		'SELECT id, first_name, middle_name, last_name FROM model_exemple.authors;',
	);
...
const findById = async (id) => {
  ...
  const query = 'SELECT first_name, middle_name, last_name FROM model_exemple.authors WHERE id = ?;';
  const [authorData] = await connection.query(query, [id]);

  if (authorData.length === 0) return null; // Para os casos que o id não encontra um equivalente no db, a resposta retornada sera nula 
...
const createAuthor = async (firstName, middleName, lastName) => connection.query(
  'INSERT INTO model_exemple.authors (first_name, middle_name, last_name) VALUES (?,?,?)',
